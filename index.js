...
import { setupMaster, fork } from 'cluster' // cluster is imported but not used (setupMaster, fork)
import { watchFile, unwatchFile } from 'fs'
import cfonts from 'cfonts'
import { createRequire } from 'module'
import { fileURLToPath, pathToFileURL } from 'url'
import { platform, tmpdir as osTmpdir } from 'process' // Changed tmpdir to osTmpdir to avoid conflict
...
let isInit = true;
let handler = await import('./handler.js'); // Ensure handler.js exports a default or named 'handler'
...
global.reloadHandler = async function(restatConn) {
  try {
    ...
    if (HandlerModule && (HandlerModule.handler || HandlerModule.default)) {
        ...
        if (typeof handler !== 'function') { // common mistake is to export an object with handler inside
    ...
    // Keep using the old handler if the new one fails to load
  }

  if (restatConn) {
    const oldChats = global.conn?.chats || {}; // Safely access old chats
    try {
    ...
    if (global.conn?.ev) global.conn.ev.removeAllListeners(); // Remove listeners from old conn
    
    global.conn = makeWASocket(connectionOptions); // Re-initialize with existing options
    isInit = true; // Mark as re-initializing
  }

  // Re-bind events, ensuring 'this' context is correct for handler and saveCreds
  if (!isInit || restatConn) { // If it's not the very first init or if we are restating connection
    if (global.conn.ev) { // Ensure ev exists
        global.conn.ev.off('messages.upsert', global.conn.handler); // Use bound handler
        global.conn.ev.off('connection.update', global.conn.connectionUpdate);
        global.conn.ev.off('creds.update', global.conn.credsUpdate);
    }
  }
  
  // Bind methods to the new conn object if it was recreated
  global.conn.handler = handler.bind(global.conn);
  global.conn.connectionUpdate = connectionUpdate.bind(global.conn);
  global.conn.credsUpdate = saveCreds.bind(global.conn); // saveCreds doesn't need 'true' here as it's bound to conn
...
async function _quickTest() {
  const test = await Promise.all([
    ...
    return Promise.race([
      new Promise((resolve) => {
        p.on('close', (code) => {
          resolve(code === 0 || (p.spawnargs.includes('color') && code === 1) ); // ffmpeg webp might exit 1 on success with pipe
        });
...
function clearTmp() {
  ...
  if (!existsSync(tmpDir)) {
      mkdirSync(tmpDir, {recursive: true}); // Create tmp if it doesn't exist
...
function purgeSessionSB() { // Clears pre-keys from JadiBot session subdirectories
  ...
  try {
    const botDirectories = readdirSync(jadiBotsPath).filter(f => statSync(join(jadiBotsPath, f)).isDirectory());
    let totalDeleted = 0;
      ...
      filesAndFolders.forEach(item => {
        ...
        try {
          if (statSync(itemPath).isFile() && item !== 'creds.json' && !item.startsWith('pre-key-')) {
            // Add more conditions if other files need to be preserved
            // This function is now for "other residual files" not covered by session purges
            // Example: old logs, non-essential data files etc.
            // For now, it's broad. Be careful or make it more specific.
            // unlinkSync(itemPath); // CAUTION: This is broad. Commenting out by default.
            // totalFilesDeleted++;
            // console.log(chalk.bold.green(`\n╭» ❍ ARCHIVO RESIDUAL ❍\n│→ ${item} BORRADO CON ÉXITO DE ${dir}\n╰― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ⌫ ♻`));
          }
        } catch (e) {
          // console.log(chalk.bold.red(`\n╭» ❍ ARCHIVO ❍\n│→ ${item} NO SE LOGRÓ BORRAR DE ${dir}\n╰― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ― ⌫ ✘\n` + e.message));
        }
      });
    } catch (err) {
...
// `redefineConsoleMethod` is not used in the provided code, can be removed if not needed elsewhere.
...
/*
// Interval for purgeOldFiles - disabled by default as it can be risky if not configured carefully
setInterval(async () => {
  if (global.stopped === 'close' || !conn || !conn.user) return
  await purgeOldFiles()
}, 1000 * 60 * 15) // 15 min, for other old files
*/

_quickTest()
  .then(() => conn.logger.info(chalk.bold(`✦ PRUEBAS RÁPIDAS COMPLETADAS ✦\n`.trim())))
  .catch(e => console.error(chalk.red("Error en quickTest:"), e));

async function isValidPhoneNumber(number) {
  try {
    let normalizedNumber = number.replace(/\s+/g, ''); // Remove spaces
    // Specific normalization for Mexican numbers (example)
...
// Ensure settings.js defines:
// global.APIs, global.APIKeys
// global.sessions (string, e.g., 'miku_sessions')
// global.jadi (string, e.g., 'JadiBots')
// global.nameqr (string, e.g., 'MikuBot-QR')
// global.botNumber (string, e.g., '+1234567890')
// global.Starlights (boolean, true/false for enabling JadiBot feature)
// global.opts['autocleartmp'] (boolean, can be set via command line too)
